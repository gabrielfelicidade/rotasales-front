            ...
            {sale.items.length > 0 && items.length > 0 ? sale.items.map((item, index) => {
                ...
                return (
                    <Grid item xs={12} style={{ display: 'flex', flexWrap: 'wrap', gap: '20px', alignItems: 'center', justifyContent: 'center' }} key={`grid-item-${index}`}>
                        <Grid item xs={12} md={8} lg={6} style={{ display: 'flex', flexWrap: 'wrap', gap: '20px', alignItems: 'center', justifyContent: 'center' }}>
                            <FormControl sx={{ minWidth: 80 }}
                                style={{ width: '100%' }}>
                            ...
                            <TextField type="number" value={item.amount} style={{ flex: '1' }} onChange={(e) => handleChangeItemAmount(e, index)} id="amount" label="Quantidade" variant="outlined" />
                            <TextField disabled value={formatCurrency(totalAmount)} style={{ flex: '1' }} id="total" label="Valor Total" variant="outlined" />
                        </Grid>
                    </Grid>
                )
            }) : null}
